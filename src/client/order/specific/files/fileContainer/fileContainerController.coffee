controller = (simpleDialogService, RESTHelperService, $window) ->
    ...
    ctrl.action = (event) ->
        ...
        if ctrl.remove
            ...
            delete ctrl.file
            ctrl.order.action = "delete"
            (if ctrl.order[layer].view? then delete ctrl.order[layer].view) for layer in ctrl.viewLayers
            preview()
        else
    ...
    ctrl.upload = -> if typeof ctrl.file is "object" and ctrl.file?
        if ctrl.file.size < 10000000
            ctrl.order.files[ctrl.layer] = ctrl.file
            ctrl.order.action = "upload"
            preview()
