...
wipeFromCollection = (collection) -> (req, res, next) ->
...
wipeFromDB = [
    (req, res, next) ->
        ...
        (models[0].find user: req.userID).exec().then ((docs) ->
            for doc in docs
...
wipeFromDB.push (req, res, next) -> (userModel.remove _id: req.userID).exec().then (-> next()), next
...
module.exports =
    get: (req, res, next) ->
        ((userModel.find {}).sort admin: "desc").exec().then ((docs) -> query res, users: docs), next

    post: (req, res, next) -> (userModel.create req.body.user).then ((doc) -> query res, doc), next

    patch: (req, res, next) ->
        id = if req.body.id is "id" then req.user.user._id else req.body.id
        find = (userModel.findByIdAndUpdate id, $set: req.body.user, {new: yes})
        find.exec().then ((doc) -> query res, doc), next

    put: (req, res, next) ->
        (userModel.findOne email: req.body.taken).exec().then ((doc) -> query res, taken: doc?), next
